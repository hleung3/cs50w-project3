...
class Order(models.Model):
    ...
    def get_order_price(self):
        ...
        # Pizzas
        price += sum([x.get_total_price() for x in OrderPizza.objects.filter(order__id=self.id)])

        # Subs
        price += sum([x.get_total_price() for x in OrderSub.objects.filter(order__id=self.id)])

        # Pastas
        price += sum([x.get_total_price() for x in OrderPasta.objects.filter(order__id=self.id)])

        # Salads
        price += sum([x.get_total_price() for x in OrderSalad.objects.filter(order__id=self.id)])

        # Dinner Platters
        price += sum([x.get_total_price() for x in OrderDinnerPlatter.objects.filter(order__id=self.id)])
...
class OrderDinnerPlatter(models.Model):
    ...
    quantity = models.PositiveIntegerField()
    def get_unit_price(self):
        return self.dinner_platter.price
    def get_total_price(self):
